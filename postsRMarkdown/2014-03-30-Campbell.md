...
```
## Subsetting
An important aspect of image processing is reducing the effective size of the image to allow for easier distrubtion.
```
nrow(coral)
ncol(coral)
...
After exploring the options available for photo processing I wanted to do something with a greater connection with my own interest. I set out to create a function that would create an NDVI from two Landsat images then average the images together and output them.NDVI is a relationship between visible and infrared reluctance that has been show to correlate with vegetation biomass. If you are interested in following along from this point the images are available at [Dropbox](https://www.dropbox.com/sh/fvtaeaftlsavrag/xQEMxuW7Vg "Landsat images and Pug"). All images are in a zip but this is a large file so be warned.
...
I then analyzed these images in ArcMap to insure they had been averaged on a pixel by pixel level. They need to be georeferenced to a higher resolution image, due to differences in parameters during each image collection date. 
...
```
That is the image, and the red band of the image as a gray scale image. I want to query the image with a certain threshold and get back a Boolean matrix that can be turned into a raster. I am sure there is a simple way to do this that I am missing.


## Conclusion

Accomplishing image processing similar to Mat lab is possible using R packages and base R. Especially useful are the image read/write packages that allow you to manipulate images as an array or matrix. 
